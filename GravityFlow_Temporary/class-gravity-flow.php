 ...
 * @copyright   Copyright (c) 2015-2025 Rocketgenius, Inc.
...
use Gravity_Flow\Gravity_Flow\Rest_API\V2\REST_API_Service_Provider;
...
if ( class_exists( 'GFForms' ) ) {
	...
	class Gravity_Flow extends GFFeedAddOn {
		...
		/**
		 * Hash of the assignee settings for a workflow step.
		*
		 * @var null|string
		 */
		private $_assignee_settings_md5 = null;
		...
		/**
		 * The minimum version of Gravity Forms required to support all features.
		 *
		 * The Framework will display an appropriate message on the plugins page if necessary
		 *
		 * @var string
		 */
		protected $_min_compatible_gravityforms_version = '2.9';
		...
		/**
		* Declares the instance of this property.
		*
		* @var null
		*/
		private $step_order = null;
		...
		*/
		...
		public function init() {
			...
			add_filter( 'gform_feed_settings_before_fields', array( $this, 'add_step_navigation' ), 10, 2 );
		...
		*/
		public function register_services() {
			$container = \GFForms::get_service_container();
			// Rest API V2.
			require_once 'includes/rest-api/v2/class-rest-api-service-provider.php';
			$container->add_provider( new REST_API_Service_Provider() );
		}
		...
		public function init_admin() {
			...
			add_filter( 'gform_noconflict_styles', array( $this, 'register_gravityflow_noconflict_styles' ) );
			...
			Locking\Locking::get_instance()->enqueue_scripts();
		}
		...
		public function init_ajax() {
			...
			add_action( 'wp_ajax_gravityflow_fetch_assignees', array( $this, 'fetch_assignees' ) );

			Locking\Locking::get_instance();
		}
...
PRIMARY KEY  (id)
			...
			$scripts = array(
				...
				array(
					...
					'strings' => array(
						...
						'operator_labels' => array(
							'is'         => esc_html__( 'Is', 'gravityflow' ),
							'isnot'      => esc_html__( 'Is Not', 'gravityflow' ),
							'>'          => esc_html__( 'Greater Than', 'gravityflow' ),
							'<'          => esc_html__( 'Less Than', 'gravityflow' ),
				...
				array(
					...
					'strings' => array(
						...
						'ajaxSearchEnabled'         => $this->is_ajax_search_enabled(),
						'ajax_search_nonce'         => wp_create_nonce( 'gravity_flow_ajax_search_nonce' ),
				...
				array(
					...
					'strings' => array(
						...
						'cannotDelete'   => esc_html__( 'One or more of the steps you selected has entries associated with it.  You cannot delete these steps.', 'gravityflow' ),
					),
				),
				array(
		...
		/**
		 * Add required styles to the noconflict list.
		 *
		 * @since 2.9.5
		 *
		 * @param array $styles The list of styles to be added.
		 *
		 * @return mixed
		 */
		public function register_gravityflow_noconflict_styles( $styles ) {
			$styles[] = 'gravityflow_dashicons';
			return $styles;
		}
		...
		public function enqueue_frontend_scripts() {
			...
			if ( isset( $wp_query->posts ) && is_array( $wp_query->posts ) ) {
				...
				if ( $shortcode_found ) {
					...
					wp_enqueue_script( self::VENDOR_JS_THEME, $this->get_base_url() . "/assets/js/dist/vendor-theme{$this->min()}.js", array( 'gform_gravityforms_libraries', 'gform_gravityforms_utils', 'gform_gravityforms_react_utils', 'gform_gravityforms_admin_components' ), $this->_version, true );
		...
		/**
		 * Fetches the list of users, roles, and fields, with ajax.
		 *
		 * @since 3.0.0
		 *
		 * @return array Array of users, roles, and fields.
		 */
		public function fetch_assignees() {
			check_ajax_referer( 'gravity_flow_ajax_search_nonce', 'nonce' );

			if ( empty( rgpost( 'search' ) ) ) {
				$results = gravity_flow()->get_users_as_choices( rgpost( 'form_id' ) );
			...
			$results = gravity_flow()->get_users_as_choices( rgpost( 'form_id' ), $search );
		 ...
		 *
		 * @param int|null    $formId The form ID to get the choices for. Included if called in an AJAX contenxt.
		 * @param string|null $search The search term to filter the users by. Included if called in an AJAX context.
		 *
		 * @since unknown
		 * @since 3.0.0 Added support for ajax search.
		 *
		 * @return array
		 */
		public function get_users_as_choices( $formId = null, $search = null ) {
			static $choices;
			...
			if ( $total_accounts > $args['number'] ) {
				...
				$notification_types = $this->get_active_notification_types();

				$current_users       = array();
				$exclude_account_ids = array();
				// Get all types of notification users and assignees.
				foreach ( $notification_types as $type ) {
					// Merge the current selected assignees into the array so they show up in ajax mode.
					$select_key   = ( $type === 'assignee' ) ? 'assignees' : $type . '_notification_users';
					$select_value = rgar( $feed_meta, $select_key );
					if ( ! empty( $select_value ) ) {
						$current_users = array_merge( $current_users, $select_value );
					}

					// Merge the current conditional routing assignees into the array so they show up in ajax mode.
					$routing_key           = ( $type === 'assignee' ) ? 'routing' : $type . '_notification_routing';
					$current_users_routing = rgar( $feed_meta, $routing_key );
					...
					// Add the users in the $_POST to the array to make sure they're present on load in the ajax context.
					$current_users = $this->mergePostedUsers( $current_users, $type );
				}
			...
			if ( ! isset( $choices[ $key ] ) ) {
				$role_choices = Gravity_Flow_Common::get_roles_as_choices( true, true, false, $search );

				// Get a user list.
				if ( $this->is_ajax_search_enabled( $total_accounts ) ) {
					if ( empty( $search ) ) {
						$accounts = array();
					} else {
						$args['search'] = '*' . $search . '*';
				...
				$form_id = $formId ? $formId : absint( rgget( 'id' ) );

				$form = GFAPI::get_form( $form_id );

				$field_choices = array();

				$assignee_fields_as_choices = $this->get_assignee_fields_as_choices( $form, $search );

				if ( ! empty( $assignee_fields_as_choices ) ) {
					$field_choices = $assignee_fields_as_choices;
				}

				$email_fields_as_choices = $this->get_email_fields_as_choices( $form, $search );
		...
		/**
		 * Make sure posted users are included in the loaded choices if we're in an ajax context and not displaying all users.
		 *
		 * @param array  $current_users The current users array.
		 ...
		 * @return array|mixed
		 */
		private function mergePostedUsers( $current_users, $type ) {
			// Generate dynamic field name based on notification type.
			$field_name        = $this->get_notification_field_name( $type );
			$posted_field_name = "_gform_setting_{$field_name}";

			if ( isset( $_POST[ $posted_field_name ] ) ) {
				$current_users = array_merge( $current_users, $_POST[ $posted_field_name ] );
			}

			return array_unique( $current_users );
		}

		/**
		 * Get all active notification types from the current form/settings.
		 *
		 * @since 3.0.0
		 *
		 * @return array Array of notification types.
		 */
		private function get_active_notification_types() {
			$notification_types = array( 'assignee' );

			// Get current feed settings to look for existing notification types.
			$settings  = $this->get_feed( rgget( 'fid' ) );
			$feed_meta = rgar( $settings, 'meta', array() );

			foreach ( $feed_meta as $key => $value ) {
				if ( preg_match( '/^(.+)_notification_type$/', $key, $matches ) ) {
					$notification_types[] = $matches[1];
				}
			}

			// Look for data from the POST.
			foreach ( $_POST as $key => $value ) {
				if ( preg_match( '/^_gform_setting_(.+)_notification_type$/', $key, $matches ) ) {
					$notification_types[] = $matches[1];
				}
			}

			return array_unique( $notification_types );
		}

		/**
		 * Get the field name pattern for a notification type.
		 *
		 * @since 3.0.0
		 *
		 * @param string $notification_type The notification type (e.g. 'assignee', 'approval', etc.).
		 *
		 * @return string The field name.
		 */
		private function get_notification_field_name( $notification_type ) {
			if ( $notification_type === 'assignee' || $notification_type === 'assignees' || empty( $notification_type ) ) {
				return 'assignees';
			}

			return "{$notification_type}_notification_users";
		}

		/**
		 * Determines if the AJAX search is enabled for the users list.
		 *
		 * @since 3.0.0
		 *
		 * @param int $total_accounts The total number of accounts.
		 *
		 * @return bool
		 */
		public static function is_ajax_search_enabled( $total_accounts = null ) {
			$enabled = apply_filters( 'gravityflow_assignee_ajax_search', '__return_true' );

			if ( ! $enabled ) {
				return false;
			}

			$account_threshold = apply_filters( 'gravityflow_assignee_ajax_search_account_threshold', 150 );

			if ( ! $total_accounts ) {
				$total_accounts = Gravity_Flow_Common::get_total_accounts();
			}

			if ( $total_accounts <= $account_threshold ) {
				return false;
			}
		 ...
		 * @param null|array  $form   Null or the form to retrieve the assignee fields from.
 		 * @param null|string $search Optional search term to filter the fields by label.
		 *
 		 * @since unknown
 		 * @since 3.0.0 Added $search parameter to filter the fields by label.
 		 *
		 * @return array
		*/
		public function get_assignee_fields_as_choices( $form = null, $search = null ) {
			if ( empty( $form ) ) {
				$form_id = absint( rgget( 'id' ) );
				$form    = GFAPI::get_form( $form_id );
			}
		 ...
		 * @return array
		 */
		public function get_email_fields_as_choices( $form = null, $search = null ) {
			if ( empty( $form ) ) {
				$form_id = absint( rgget( 'id' ) );
				$form = GFAPI::get_form( $form_id );
			}
		...
		/*
		 * Add step navigation to the step editing page.
		 *
		 * @since 3.0.0
		 *
		 * @param string $before_fields The HTML to be displayed before the form fields.
		 * @param array $form The form currently being edited.
		 */
		public function add_step_navigation( $before_fields, $form ) {
			if ( 'gravityflow' !== rgget( 'subview' ) ) {
				return $before_fields;
			}

			$step_navigation = $this->get_step_navigation( $form['id'] );
			return $step_navigation . $before_fields;
		}

		/**
		 * Get the step navigation HTML.
		 *
		 * @since 3.0.0
		 *
		 * @param array $form The form currently being edited.
		 */
		private function get_step_navigation( $form ) {
			$steps = $this->get_steps( $form );
			if( ! $steps ) {
				return '';
			}
			$steps[] = $this->get_workflow_complete_step( $form );
			?>

			<article class="gform-dropdown" data-js="gflow-step-switcher">
				<span
					class="gform-visually-hidden"
					id="gflow-step-switcher-label"
				><?php esc_attr_e( 'Select a different step', 'gravityflow' ); ?></span>
				<button
					aria-expanded="false"
					aria-haspopup="listbox"
					aria-labelledby="gflow-step-switcher-label gflow-step-switcher-control"
					class="gform-dropdown__control"
					data-js="gform-dropdown-control"
					id="gflow-step-switcher-control"
					type="button"
				>
					<span class="gform-dropdown__control-text" data-js="gform-dropdown-control-text"><?php echo esc_html_e( 'Choose a step to edit', 'gravityflow' ); ?></span>
					<i class="gform-spinner gform-dropdown__spinner"></i>
					<i class="gform-icon gform-icon--chevron gform-dropdown__chevron"></i>
				</button>
				<div
					aria-labelledby="gflow-step-switcher-label"
					class="gform-dropdown__container"
					role="listbox"
					tabindex="-1"
				>
					<div class="gform-dropdown__search">
						<label for="gflow-step-switcher-search" class="gform-visually-hidden"><?php esc_attr_e( 'Search steps', 'gravityflow' ); ?></label>
						<input
							id="gflow-step-switcher-search"
							type="text" class="gform-input gform-dropdown__search-input"
							placeholder="<?php esc_attr_e( 'Search for step', 'gravityflow' ); ?>"
					...
					<div class="gform-dropdown__list-container" data-simplebar<?php echo is_rtl() ? ' data-simplebar-direction="rtl"' : ''; ?>>
						...
						<?php
							foreach ( $steps as $step ) {
								...
								$step_id = $step->get_id();
								$step_title = $step->get_name();
								$content = sprintf(
									'
										<li class="gform-dropdown__item">
											<button class="gform-dropdown__trigger" data-js="gform-dropdown-trigger" data-value="%1$d" type="button">
			...
			<script type="text/javascript">
				...
				function GFlow_ReplaceQuery(key, newValue) {
					var new_query = "";
				...
				function GFlow_SwitchStep(id) {
					id = parseInt(id);
					var new_query = GFlow_ReplaceQuery('fid', id);

					document.location = '?' + new_query;
				}

				gform.instances.stepSwitcher = new gform.components.admin.html.elements.Dropdown( {
					detectTitleLength: true,
					onItemSelect: GFlow_SwitchStep,
					reveal: 'hover',
					selector: 'gflow-step-switcher',
					showSpinner: true,
				} );
		...
		public function settings_feed_condition( $field, $echo = true ) {
			...
			if ( ! did_filter( 'gform_entry_meta_pre_render_feed_settings' ) ) {
				$form_id     = absint( rgget( 'id' ) );
		...
		/**
		 * Get a list of the bulk actions to show on the feed list page.
		 *
		 * @since 3.0.0
		 *
		...
		function get_bulk_actions() {
			$bulk_actions = array(
				'activate'   => esc_html__( 'Mark as Active', 'gravityflow' ),
				'deactivate' => esc_html__( 'Mark as Inactive', 'gravityflow' ),
				'duplicate'  => esc_html__( 'Duplicate', 'gravityflow' ),
				'delete'     => esc_html__( 'Delete', 'gravityflow' ),
			);
		...
		/**
		 * Process the bulk action on the feed list page.
		 *
		 * @since 3.0.0
		 *
		...
		public function process_bulk_action( $action ) {
			switch ( $action ) {
				...
				case 'activate':
					...
					if ( is_array( $feeds ) ) {
						foreach ( $feeds as $feed_id ) {
							$this->update_feed_active( $feed_id, 1 );
						}
					}
					break;
				case 'deactivate':
					...
					if ( is_array( $feeds ) ) {
						foreach ( $feeds as $feed_id ) {
							$this->update_feed_active( $feed_id, 0 );
						}
					}
					break;
			}
				...
				<div class="postbox">
					...
					<div id="submitcomment" class="submitbox">
						<div id="minor-publishing" style="padding:10px;">
							...
							<input type="submit" class="button" onClick="maybeTriggerHandleButtonClick( this )" name="_gravityflow_admin_action" value="<?php esc_html_e( 'Apply', 'gravityflow' ); ?>"/>
		...
		public function get_status_labels_fields( $labels ) {
			...
			foreach ( $status_labels as $status_label_key => $status_label ) {
				if ( rgar( $default_status_labels, $status_label_key ) ) {
		...
		public function status_page( $args = array() ) {
			...
			<div class="gravityflow_wrap gf_entry_wrap gravityflow_workflow_wrap gravityflow_workflow_status">
				...
				<?php if ( $args['display_header'] ) : ?>
					<h2 class="gf_admin_page_title">
						<img width="45px" height="22px" src="<?php echo esc_url( gravity_flow()->get_base_url() ); ?>/images/gravity-flow-icon-cropped.svg" style="margin-right:5px;"/>
		...
		public function maybe_process_admin_action( $form, $entry ) {
			$feedback = false;
			if ( isset( $_POST['gravityflow_admin_action'] ) && check_admin_referer( 'gravityflow_admin_action', '_gravityflow_admin_action_nonce' ) && GFAPI::current_user_can_any( 'gravityflow_workflow_detail_admin_actions' ) ) {
		...
		/**
		 * Determines if there is an active step of the specified type.
		 *
		 * @since  2.9.8
		 *
		 * @param string              $type  The step type.
		 * @param Gravity_Flow_Step[] $steps The steps for the current form.
		 *
		 * @return bool
		 */
		public function has_active_step( $type, $steps ) {
			foreach ( $steps as $step ) {
				if ( $step->get_type() === $type && $step->is_active() ) {
					return true;
				}
			}
		...
		public function get_local_dev_base_url() {
			$url = $this->get_base_url();

			if ( ! defined( 'GRAVITYFLOW_ENABLE_HMR' ) || ! GRAVITYFLOW_ENABLE_HMR ) {
		...
		public function action_admin_enqueue_scripts() {
			...
			$local_dev_url = $this->get_local_dev_base_url();
			$dev_min       = defined( 'GF_SCRIPT_DEBUG' ) && GF_SCRIPT_DEBUG ? '' : '.min';

			// Enqueue admin CSS and JS
			wp_enqueue_style( self::ADMIN_COMPONENTS_CSS,  $base_url . "/assets/css/dist/admin-components{$dev_min}.css", null, $this->_version );
			wp_enqueue_style( self::ADMIN_CSS,  $base_url . "/assets/css/dist/admin{$dev_min}.css", null, $this->_version );

			wp_enqueue_script( self::VENDOR_JS_ADMIN, $base_url . "/assets/js/dist/vendor-admin{$dev_min}.js", array( 'gform_gravityforms_libraries', 'gform_gravityforms_utils', 'gform_gravityforms_react_utils', 'gform_gravityforms_admin_components' ), $this->_version, true );
			wp_enqueue_script( self::ADMIN_JS, $local_dev_url . "/scripts-admin{$dev_min}.js", array( self::VENDOR_JS_ADMIN ), $this->_version, true );
...
AND m.meta_value='queued'";
			...
			foreach ( $form_ids as $form_id ) {
				...
				foreach ( $steps as $step ) {
					...
					foreach ( $entries as $entry ) {
						...
						if ( $step->resend_assignee_emailEnable !== '1' ) {
							continue;
						}
						...
						foreach ( $assignees as $assignee ) {
							...
							if ( $assignee_status == 'pending' ) {
								...
								if ( time() > $trigger_timestamp && $reminder_timestamp == false ) {
									$this->log_debug( __METHOD__ . '(): assignee_timestamp: ' . $assignee_timestamp . ' - ' . get_date_from_gmt( gmdate( 'Y-m-d H:i:s', $assignee_timestamp ), 'F j, Y H:i:s' ) );
									$this->log_debug( __METHOD__ . '(): trigger_timestamp: ' . $trigger_timestamp . ' - ' . get_date_from_gmt( gmdate( 'Y-m-d H:i:s', $trigger_timestamp ), 'F j, Y H:i:s' ) );
								...
								if ( time() > $trigger_timestamp && $reminder_timestamp !== false ) {
									$this->log_debug( __METHOD__ . '(): not sending first reminder to ' . $assignee->get_key() . ' for entry ' . $entry['id'] . ' because a reminder was already sent: ' . get_date_from_gmt( gmdate( 'Y-m-d H:i:s', $reminder_timestamp ), 'F j, Y H:i:s' ) );
									if ( $current_step->resend_assignee_email_repeatEnable ) {
										$repeat_days = absint( $current_step->resend_assignee_email_repeatValue );

										// Deprecated Filter - See/Use gravityflow_assignee_email_reminder_repeat_days.
										...
										if ( $repeat_days > 0 ) {
											...
											if ( time() > $repeat_trigger_timestamp ) {
												...
												$this->log_debug( __METHOD__ . '(): repeat reminder to ' . $assignee->get_key() . ' for entry ' . $entry['id'] . ' is scheduled for ' . get_date_from_gmt( gmdate( 'Y-m-d H:i:s', $repeat_trigger_timestamp ), 'F j, Y H:i:s' ) );
											}
										}
									}
								}
								if ( time() < $trigger_timestamp && $reminder_timestamp == false ) {
									$this->log_debug( __METHOD__ . '(): reminder to ' . $assignee->get_key() . ' for entry ' . $entry['id'] . ' is scheduled for ' . get_date_from_gmt( gmdate( 'Y-m-d H:i:s', $trigger_timestamp ), 'F j, Y H:i:s' ) );
								}
							}
						}
					}
		...
		public function filter_wp() {
			...
			if ( isset( $_GET['gflow_access_token'] ) ) {
				...
				if ( ! empty( $token ) && ! isset( $token['scopes']['action'] )&& ! is_user_logged_in() ) {
					...
					setcookie( 'gflow_access_token', $sanitized_cookie, 0, $this->get_cookie_path(), $domain, $secure, true );
		...
		public function filter_wp_login() {
			...
			$secure = ( 'https' === parse_url( home_url(), PHP_URL_SCHEME ) );
			$domain = parse_url( home_url(), PHP_URL_HOST );
			setcookie( 'gflow_access_token', '', -1, $this->get_cookie_path(), $domain, $secure, true  );
		}
		...
		/**
		 * Makes the entry meta available for use with the step condition.
		 *
		 * @since 2.9.10
		 *
		 * @param array            $entry_meta An empty array or the entry meta fields supported for use with the step condition.
		 * @param array            $form       The form the logic is being evaluated for, or the feed is being created or edited for.
		 * @param null|GFFeedAddOn $addon      Null during evaluation, or the instance of the add-on the feeds settings page belongs to.
		 *
		 * @return array
		 */
		 public function filter_conditional_logic_entry_meta( $entry_meta, $form, $addon = null ) {
			if ( ! is_null( $addon ) && $addon !== $this ) {
				return $entry_meta;
			}

			$step_id = $this->is_feed_edit_page() ? absint( $this->get_current_feed_id() ) : 0;

			return array_merge( $this->get_feed_condition_entry_meta( rgar( $form, 'id' ), $step_id, $entry_meta ), $this->get_feed_condition_entry_properties() );
		}
		 ...
		 * @since 2.9.10    Added the $entry_meta param
		 *
		 * @param int   $form_id    The form ID.
		 * @param int   $step_id    The step ID.
		 * @param array $entry_meta The entry meta, if available.
		 *
		 * @return array
		 */
		public function get_feed_condition_entry_meta( $form_id = 0, $step_id = 0, $entry_meta = array() ) {
			if ( empty( $entry_meta ) ) {
		 ...
		 *
		 * @since 1.7.1-dev
		 * @since 2.9.10 Updated to use GFCommon::evaluate_conditional_logic() with GF 2.9 or greater.
		 *
		...
		public function evaluate_conditional_logic( $logic, $form, $entry ) {
			if ( $this->is_gravityforms_supported( '2.9.0-rc-2' ) ) {
				add_filter( 'gform_entry_meta_pre_evaluate_conditional_logic', array( $this, 'filter_conditional_logic_entry_meta' ), 1, 2 );
				add_filter( 'gform_rule_pre_evaluation', array( $this, 'filter_rule_pre_evaluation' ), 99, 5 );
				$result = GFCommon::evaluate_conditional_logic( $logic, $form, $entry );
				remove_filter( 'gform_entry_meta_pre_evaluate_conditional_logic', array( $this, 'filter_conditional_logic_entry_meta' ), 1 );
				remove_filter( 'gform_rule_pre_evaluation', array( $this, 'filter_rule_pre_evaluation' ), 99 );
		...
		/**
		 * Processes merge tags in the rule value before the rule is evaluated.
		 *
		 * @since 2.9.10
		 *
		 ...
		 * @return array
		 */
		public function filter_rule_pre_evaluation( $rule, $form, $logic, $field_values, $entry ) {
			if ( empty( $rule['value'] ) ) {
				return $rule;
			}
