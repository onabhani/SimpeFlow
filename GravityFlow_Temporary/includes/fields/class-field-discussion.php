...
class Gravity_Flow_Field_Discussion extends GF_Field_Textarea {
	...
	public function format_discussion_item( $item, $format, $entry_id ) {
		...
		if ( $format === 'html' ) {
			$item_classes = $this->is_form_editor() ? ' gform-theme__disable-reset ginput_container' : '';
...
%s
            ...
            $return .= sprintf( '<div id="gravityflow-discussion-item-%s" class="gravityflow-discussion-item%s">%s</div>', sanitize_key( $item['id'] ), esc_attr( $item_classes ), $content );
		} elseif ( $format === 'text' ) {
		...
		/**
		 * Allow the discussion item markup to be modified before display.
		 *
		 * @param string                        $discussion_item The discussion item markup.
		 * @param array                         $item            The discussion item data.
		 * @param string                        $format          The requested format for the value; html or text.
 		 * @param int|null                      $entry_id        The ID of the entry currently being edited or null in other locations.
		 * @param Gravity_Flow_Field_Discussion $this            The field currently being processed.
		 *
		 * @since 3.0.0
		 * 
		 * @return string The modified discussion item markup.
		 */

		$return = apply_filters( 'gravityflow_discussion_item', $return, $item, $format, $entry_id, $this );
