...
abstract class Gravity_Flow_Step extends stdClass {
	 ...
	 * This should be a plural noun. e.g. approvals
	 *
	...
	public function get_rest_base() {
		...
		_deprecated_function( __METHOD__, '3.0.0', 'Steps only need to implement rest_process_step() to add REST API support' );
	 ...
	 *
	 * @param bool $force_refresh Whether to force a refresh of the entry from the database.
	 *
	 * @return array|null
	 */
	public function get_entry( $force_refresh = false ) {
		$var_entry = $this->_entry;
		if ( empty( $this->_entry ) || $force_refresh ) {
			$this->refresh_entry();
		}
	 ...
	 * @since 3.0.0
	 *
	 * @return Gravity_Flow_Assignee | bool Returns the assignee object or false if no assignee found.
	 */
	 ...
	 * @param string $assignee_key The assignee key.
	 *
	 * @return bool True if the assignee key is assigned to this step. False otherwise.
	 */
	public function has_assignee( $assignee_key ) {
		$assignees = $this->get_assignees();
		foreach ( $assignees as $assignee ) {
			if ( $assignee_key === $assignee->get_key() ) {
				return true;
			}
		}
		return false;
	...
	public function evaluate_routing_rule( $routing_rule ) {
		...
		if ( in_array( $field_id, $entry_meta_keys ) || in_array( $field_id, $entry_properties ) ) {
			...
			$is_value_match = GFFormsModel::is_value_match( $field_value, rgar( $routing_rule, 'value' ), rgar( $routing_rule, 'operator' ), $source_field, $routing_rule, $form );
		}
	...
	public function send_notifications( $assignees, $notification ) {
		...
		foreach ( $assignees as $assignee ) {
			...
			if ( $assignee instanceof Gravity_Flow_Assignee ) {
				$assignee->send_notification( $notification );
			}
		}
	}
