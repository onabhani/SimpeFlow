...
class Gravity_Flow_Common {
	 ...
	 * @param bool $search        The search term if we're searching for roles. Default is null.
	 *
	 * @return array
	 *
	 * @since 1.4.2-dev
	 * @since 3.0.0 Added `$search` parameter to filter roles by query.
	 */
	public static function get_roles_as_choices( $prefix_values = true, $reverse = false, $frontend = false, $search = null ) {
	...
	public static function get_users_args() {
		$number_to_retrieve = apply_filters( 'gravityflow_assignee_ajax_search_account_threshold', 150 );

		$default_args = array(
			'orderby' => array( 'display_name', 'user_login' ),
			'fields'  => array( 'ID', 'display_name', 'user_login' ),
			'number'  => $number_to_retrieve,
		);
