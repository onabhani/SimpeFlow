...
class Gravity_Flow_Entry_Editor {
	...
	public function remove_page_fields( $form ) {
		...
		foreach ( $form['fields'] as $key => $field ) {
			...
			// Register calculation dependencies even if the field is not editable.
	...
	public function is_dependency( $field_or_id ) {
		...
		// Check if this field is in the dependencies array.
		$is_dependency = in_array( $field_id, $this->_dependencies );
		
		// If field is not already a dependency, check if it has a calculation that might make it a dependency.
		if ( ! $is_dependency && is_object( $field_or_id ) && $field_or_id->has_calculation() ) {
			$is_dependency = true;
		}
	...
	public function can_remove_field( $field ) {
		// Prevent removal of calculated fields to ensure their dependencies work correctly.
		$is_calculation_field = $field->has_calculation();

		$can_remove_field = ! ( $this->is_editable_field( $field ) || $this->is_display_field( $field ) || $this->is_dependency( $field ) || $this->is_pricing_field_required( $field ) || $field->type == 'html' || $is_calculation_field ) && empty( $field->conditionalLogicFields );
	 ...
	 *
	 * @since 2.1.2-dev
	 * @since 3.0 Updated to use GF_Field_File_Upload::add_population_file_url() with GF 2.9.18+.
	 *
	...
	public function get_post_image_value( $value, $field ) {
		...
		$array      = explode( '|:|', $value );
		$value      = array(
			$field_id . '.1' => rgar( $array, 1 ), // Title.
			$field_id . '.2' => rgar( $array, 4 ), // Alt.
			$field_id . '.4' => rgar( $array, 2 ), // Caption.
			$field_id . '.7' => rgar( $array, 3 ), // Description.
		);

		if ( method_exists( $field, 'populate_file_url' ) ) {
			$field->populate_file_url( rgar( $array, 0 ) );
		} else {
			...
			if ( ! isset( GFFormsModel::$uploaded_files[ $form_id ][ $input_name ] ) ) {
				...
				GFFormsModel::$uploaded_files[ $form_id ][ $input_name ] = $path_info['basename'];
			}
		}
	...
	public function get_non_editable_field( $field ) {
		...
		$is_calc_field                = $field->has_calculation();

		// Always include the field input for calculated fields to ensure JS calculations work.
		if ( $conditional_logic_dependency || $this->is_dependency( $field ) || $this->is_pricing_field_required( $field ) || $is_calc_field ) {
			$html = $field->get_field_input( $this->form, $value, $this->entry );
		}
	...
	public function is_display_field( $field, $is_init = false ) {
		...
		// For fields with conditional logic and dynamic conditional logic enabled, check if they should be display fields even if initially hidden
		if ( ! empty( $field->conditionalLogic ) && $this->_is_dynamic_conditional_logic_enabled ) {
			$display_fields_mode = $this->step ? $this->step->display_fields_mode : 'all_fields';
			
			if ( $display_fields_mode === 'all_fields' ) {
				$display_field = true;
			} elseif ( $display_fields_mode !== 'all_fields' && ! empty( $this->step->display_fields_selected ) && is_array( $this->step->display_fields_selected ) ) {
				$is_selected = in_array( $field->id, $this->step->display_fields_selected );
				// Respect the display fields mode set on the step.
				$display_field = ( $display_fields_mode === 'selected_fields' && $is_selected ) || ( $display_fields_mode === 'all_fields_except' && ! $is_selected );
			} else {
				$display_field = false;
			}
		} else {
			// For fields without conditional logic, use the standard check
