...
- Shows the current entry creator and a `<select>` of eligible users.
...
Args are passed directly to `get_users()`. Default restricts to users with `gravityforms_edit_entries` capability, ordered by display name, capped at 500.
...
- No Select2 in v1 — native `<select>`. Revisit if a user list grows past a few hundred.

## Future work

- Arabic translations for all strings marked with the `simpleflow` text domain.
- Optional Select2 wrapping when the selectable user count exceeds a threshold.
